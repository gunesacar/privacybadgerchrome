...
function setPluginErrorMsg(errMsg){
  console.error(errMsg);
}


/**
 * Get plugin state from localStorage and populate select boxes
 */
function updatePluginList(){
  Utils.getInstalledPlugins(function(plugins, error){
    if (error === undefined){
      ...
      for(var i=0; i < plugins.length; i++){
        plugin = plugins[i];
        // console.log(plugin);
        var pluginDiv = $('<div class="plugin-div" id=' +plugin.id + '-div>').appendTo('#plugin-controls');
        var div = $('<div class="plugin-desc">' + plugin.description +'</div>').appendTo(pluginDiv);
        var sel = $('<select class="plugin-selector" id=' +plugin.id + '>').appendTo(pluginDiv);
        sel.append($('<option value="allow" selected="selected">'+i18n.getMessage("options_plugin_allow")+'</option>'));
        sel.append($('<option value="block">'+i18n.getMessage("options_plugin_block")+'</option>'));
        if(blockedPlugins[plugin.id]){
          sel.val("block");
        }
      }
      $('.plugin-selector').change(function() {
        console.log("Plugin setting changed", this.id, this.value);
        Utils.setPluginBlockingState(this.id, this.value);
      });
    }else{
      setPluginErrorMsg(error);
    }
  });
